...
from .attachments import attach_pending
...
def create_ticket(db: Session, customer, *, category: str, subject: str, description: str, notifications_enabled: bool, attachment_ids: list[int] | None = None, draft_token: str | None = None, attachment_dir: str = "/share-manager/attachments") -> SupportTicket:
    ...
    msg = SupportTicketMessage(ticket_id=row.id, author_type="customer", author_label=customer.name, body=description, visible_to_customer=True)
    db.add(msg); db.flush()
    attach_pending(db, ticket=row, message=msg, attachment_ids=attachment_ids or [], storage_dir=attachment_dir, uploader_type="customer", uploader_customer_id=customer.id, draft_token=draft_token)
...
def add_customer_reply(db: Session, ticket: SupportTicket, customer, body: str, *, attachment_ids: list[int] | None = None, attachment_dir: str = "/share-manager/attachments") -> SupportTicketMessage:
    body=(body or "").strip()
    if ticket.status == "closed": raise ValueError("Closed tickets cannot be replied to")
    if (not body and not attachment_ids) or len(body)>10000: raise ValueError("Add a reply or attachment; reply text must be 10,000 characters or fewer")
    since=datetime.utcnow()-timedelta(hours=1)
    ...
    db.add(msg); db.flush(); attach_pending(db, ticket=ticket, message=msg, attachment_ids=attachment_ids or [], storage_dir=attachment_dir, uploader_type="customer", uploader_customer_id=customer.id); ticket.admin_unread=True; ticket.updated_at=datetime.utcnow()
...
def add_admin_reply(db: Session, ticket: SupportTicket, body: str, actor: str, *, attachment_ids: list[int] | None = None, attachment_dir: str = "/share-manager/attachments") -> SupportTicketMessage:
    body=(body or "").strip()
    ...
    db.add(msg); db.flush(); attach_pending(db, ticket=ticket, message=msg, attachment_ids=attachment_ids or [], storage_dir=attachment_dir, uploader_type="admin"); ticket.customer_unread=True; ticket.admin_unread=False; ticket.updated_at=datetime.utcnow()
...
def add_internal_note(db: Session, ticket: SupportTicket, body: str, actor: str, *, attachment_ids: list[int] | None = None, attachment_dir: str = "/share-manager/attachments") -> SupportTicketMessage:
    body=(body or "").strip()
    if (not body and not attachment_ids) or len(body)>10000: raise ValueError("Add an internal note or attachment; note text must be 10,000 characters or fewer")
    msg=SupportTicketMessage(ticket_id=ticket.id, author_type="internal", author_label=actor, body=body, visible_to_customer=False)
    db.add(msg); db.flush(); attach_pending(db, ticket=ticket, message=msg, attachment_ids=attachment_ids or [], storage_dir=attachment_dir, uploader_type="admin"); ticket.updated_at=datetime.utcnow(); db.add(AuditLog(actor=actor, action="ticket.internal_note", target_type="support_ticket", target_id=ticket.reference, detail="Internal note added")); db.commit(); db.refresh(msg); return msg
